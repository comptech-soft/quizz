...
<script>
    ...
    */
    ...
    export default {
        ...
        mounted()
        {
            let vm = this, count_trys = 0, i = setInterval( () => {
                if(  $('#' + this.field).length ===  1)
                {
                    clearInterval(i);
                    vm.instance = 
                        JQueryHelpers.DatepickerById(this.field, this.datepicker)
                        .on('changeDate', function(e) {
                            // console.log(e.date);
                            vm.updateValue(Moment(e.date).format('DD.MM.YYYY'))
                            // return vm.value = Moment(e.date).format('DD.MM.YYYY')
                        });
                }
            }, 250);
